...
public class PQueue <T extends Comparable<T>> {
    ...
    // This map keeps track to the possible idices a particular
    ...
    // Construct a priority queue wtin an initial capacity
    public PQueue(int sz) {
        this.heap = new ArrayList<>(sz);
    }

    // Construct a priority queue using heapify in O(n) time, a great explanation can be found in
    ...
    // Return true/false depending on if the priority queue is empty
    ...
    private void swim(int k) {
        ...
        // k - 1 because arrays is zero based
        // divide by 2 because that is the formula for
        // binary trees implemented by array
        int parent = (k-1) / 2;
    ...
    public boolean isMinHeap(int k) {
        ...
        // both of its children left if they exist
    ...
    // Add a node value its index to the map
    ...
    // Note: If a value exists multiple in the heap the highest
    // index is returned (this has arbitrarily been chosen)
    private @Nullable Integer mapGet(T value) {
        TreeSet<Integer> set = map.get(value);
